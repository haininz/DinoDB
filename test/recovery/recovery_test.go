...
func createTable(t *testing.T, db *database.Database, rm *recovery.RecoveryManager, tableType database.IndexType) string {
	...
	rm.Table(string(tableType), tableName)
	return tableName
}
...
func TestRecovery(t *testing.T) {
	...
	t.Run("AbortStress", testAbortStress)
	t.Run("InsertNoCommit", testInsertNoCommit)
	t.Run("InsertCommit", testInsertCommit)
	t.Run("InsertDeleteCommit", testInsertDeleteCommit)
	t.Run("InsertCommitDelete", testInsertCommitDelete)
	t.Run("InsertCommitUpdate", testInsertCommitUpdate)
	t.Run("InsertCheckpointCommitUpdate", testInsertCheckpointCommitUpdate)
	t.Run("InsertCommitCheckpointUpdate", testInsertCommitCheckpointUpdate)
	t.Run("MultipleTablesOneClient", testMultipleTablesOneClient)
	t.Run("MultipleTransactions", testMultipleTransactions)
	t.Run("ManyTablesAndTransactions", testManyTablesAndTransactions)
	t.Run("MultiInsertCheckpointing", testMultiInsertCheckpointing)
	t.Run("MultiInsertCommitCheckpointing", testMultiInsertCommitCheckpointing)
...
func testAbortStress(t *testing.T) {
	...
	for i := int64(0); i < 100; i++ {
		insertIntoTable(t, db, tm, rm, clientId, tableName, i, 0)
	}
...
func testInsertNoCommit(t *testing.T) {
...
func testInsertCommitDelete(t *testing.T) {
	// Define the test cases. Maps subtest name to the number of entries
	tests := map[string]int64{
		"Single": 1,
		"Many":   500,
...
func testInsertCommitCheckpointUpdate(t *testing.T) {
	db, tm, rm, clientId := setupRecovery(t, "")
	// Before crash
	tableName := createTable(t, db, rm, database.BTreeIndexType)
	startTransaction(t, db, tm, rm, clientId)
...
func testMultipleTransactions(t *testing.T) {
	// Define the test cases. Maps subtest name to the number of transactions
	tests := map[string]int{
		"Two":   2,
		"Fifty": 50,
	}

	for name, numTransactions := range tests {
		t.Run(name, func(t *testing.T) {
			db, tm, rm, _ := setupRecovery(t, "")
			transactionToCommit := rand.Intn(numTransactions)
			// Before crash
			tableName := createTable(t, db, rm, database.BTreeIndexType)
			uuids := make([]uuid.UUID, numTransactions)
			// Insert a new key for each uuid in a new transaction
			valToInsert := rand.Int63()
			for i := 0; i < numTransactions; i++ {
				id := uuid.New()
				uuids[i] = id
				startTransaction(t, db, tm, rm, id)
				insertIntoTable(t, db, tm, rm, id, tableName, int64(i), valToInsert)
			}
			// Only commit a single transaction
			commitTransaction(t, db, tm, rm, uuids[transactionToCommit])

			db, tm, rm = crashAndRecover(t, db.GetBasePath())
			// After crash, only should find insert for the transaction that committed
			for i, id := range uuids {
				startTransaction(t, db, tm, rm, id)
				if i == transactionToCommit {
					checkFind(t, db, tm, id, tableName, int64(i), valToInsert)
				} else {
					checkFindFails(t, db, tm, id, tableName, int64(i))
				}
			}
		})
	}
...
func testManyTablesAndTransactions(t *testing.T) {
	db, tm, rm, _ := setupRecovery(t, "")
	numClients := 50
	transactionToCommit := rand.Intn(numClients)
	uuids := make([]uuid.UUID, numClients)
	tableNames := make([]string, numClients)
	// Create a new table for each uuid and insert a new key in a new transaction
	valToInsert := rand.Int63()
	...
	for i, id := range uuids {
		startTransaction(t, db, tm, rm, id)
		if i == transactionToCommit {
			checkFind(t, db, tm, id, tableNames[i], int64(i), valToInsert)
		} else {
			checkFindFails(t, db, tm, id, tableNames[i], int64(i))
		}
	}
}
...
func testMultiInsertCommitCheckpointing(t *testing.T) {
	db, tm, rm, clientId := setupRecovery(t, "")
	numEntries := int64(500)
	// Before crash
	tableName := createTable(t, db, rm, database.BTreeIndexType)
	...
	db, tm, rm = crashAndRecover(t, db.GetBasePath())
	// After crash, should be able to find all entries since they committed
	startTransaction(t, db, tm, rm, clientId)
	for i := int64(0); i < numEntries; i++ {
		checkFind(t, db, tm, clientId, tableName, i, i%utils.Salt)
	}
